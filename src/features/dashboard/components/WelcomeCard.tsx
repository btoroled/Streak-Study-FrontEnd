...
export default function WelcomeCard() {
  ...
  return (
    <div className="bg-gradient-to-r from-[#f97316]/10 to-[#7c3aed]/10 border border-surface-border rounded-xl p-5">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm text-text-secondary">{greeting()}</p>
          <h2 className="text-xl font-bold text-text-primary mt-0.5">{displayName} {mascot}</h2>
          {currentStreak > 0 ? (
            ...
            <p className="text-sm text-text-secondary mt-1">Empieza tu racha hoy estudiando.</p>
          )}
        </div>
